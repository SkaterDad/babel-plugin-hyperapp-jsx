  ...
  const initConfig = (path, state) => {
    ...
    if (isString(constructor)) {
      ...
      jsxObjectTransformer = jsxObj => {
        ...
        // Try finding single children which are arrays
        // We tagged them earlier in the process with an IS_ARRAY property
        const hasSingleArrayChild =
          jsxChildren.elements.length === 1 &&
          t.isIdentifier(jsxChildren.elements[0]) &&
          jsxChildren.elements[0].IS_ARRAY === true

        if (hasSingleArrayChild) {
