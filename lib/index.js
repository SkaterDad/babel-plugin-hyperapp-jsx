...
module.exports = function({ types: t }) {
  ...
  const initConfig = (path, state) => {
    ...
    if (isString(constructor)) {
      ...
      jsxObjectTransformer = jsxObj => {
        ...
        // Extract "key" attribute and attach to VNode object
        const keyAttr = jsxAttrs.properties
          ? jsxAttrs.properties.find(x => x.key.name == 'key')
          : null

        if (keyAttr != null) {
          jsxObj.properties.push(keyAttr)
        }

        // Not a component, so return raw VNode object
        return jsxObj
      }
    }
  ...
  const visitJSXElement = (path, state) => {
    ...
    const JSXSpreadChild = node => {
      // Treat all spread children as arrays
